...
suite("test_ranger_resource_table", "p2,ranger,external") {
	...
	def checkTableAccess = { catalogType, access, user, password, catalog, dbName, tableName ->
		connect("$user", "$password", "$defaultJdbcUrl") {
			...
			if (catalogType == "internal") {
				...
				executeSqlWithLogging("""INSERT INTO ${tableName} VALUES (1, 'test')""", "Error executing INSERT")
				executeSqlWithLogging("""SELECT * FROM ${tableName}""", "Error executing SELECT")
				executeSqlWithLogging("""ALTER TABLE ${tableName} ADD COLUMN age INT""", "Error executing ALTER TABLE")
				executeSqlWithLogging("""CREATE VIEW test_view AS SELECT * FROM ${tableName}""", "Error executing CREATE VIEW")
				executeSqlWithLogging("""SELECT * FROM test_view""", "Error executing SELECT VIEW")
				executeSqlWithLogging("""SHOW CREATE VIEW test_view""", "Error executing SHOW CREATE VIEW")
				executeSqlWithLogging("""DROP TABLE ${tableName}""", "Error executing DROP DATABASE")
